...
<body>
    ...
    <div class="section">
      ...
      matches.slice(0, 10).forEach(match => {
        const timeDiff = match.publishedDiffSeconds 
          ? `${Math.round(match.publishedDiffSeconds / 60)} דקות`
          : 'לא ידוע';
        
        const winner = match.betterArticle === match.article1Id 
          ? match.source1 
          : match.betterArticle === match.article2Id 
          ? match.source2 
          : 'לא הוגדר';
        ...
        <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 15px; margin: 10px 0;">
          ...
          <div style="font-size: 14px; color: #6b7280;">
            דמיון: ${Math.round(match.similarityScore * 100)}% | 
            הפרש זמן: ${timeDiff} | 
            כתבה טובה יותר: <span style="color: #059669; font-weight: 600;">${winner}</span>
          </div>
        </div>`;
      });
  ...
  const handleSendEmail = async () => {
    ...
    try {
      const html = generateHTMLReport();
      await api.sendReport(email, html, reportType);
      alert('✅ הדוח נשלח בהצלחה למייל');
      setEmail('');
    } catch (error) {
      console.error('Email error:', error);
      alert('❌ שגיאה בשליחת המייל. ייתכן שהגדרות SMTP לא מוגדרות בשרת.');
    } finally {
      setIsSending(false);
    }
  };
        ...
        <div className="space-y-4">
          ...
          <div className="pt-4 border-t border-gray-200">
            ...
            <p className="text-xs text-gray-500 mt-2">
              💡 שליחת מייל דורשת הגדרות SMTP בשרת
            </p>
          </div>
        </div>
      )}
